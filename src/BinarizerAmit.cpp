/*#include "StdAfx.h"
#include "BinarizerAmit.h"
#include "DImage.h"
#include "BinarizerNiblack.h"
using namespace std;

BinarizerAmit::BinarizerAmit(void){
	;
}
...
float BinarizerAmit::mStrokeWidthCalc(Mat mat, int x, int y){
	int i, j;
	float res = 0;
	this->setSW(1);
...
int* BinarizerAmit::setInFirstArr(Mat *im, int x, int y) {
	int pxl1[8] = { 0 };
...
int* BinarizerAmit::setInSecArr(Mat *im, int x, int y){
	int pxl1[8] = { 0 };
...
void BinarizerAmit::structuralContrast(Mat im, Mat out)
{
	int* pxl1;
	int* pxl2;
	int i, j, k, l, m;
...
int BinarizerAmit::mMin(Mat im, int x, int y){
	int i;
	int j;
	float result = 1000;
	float tmp;
...
int BinarizerAmit::mMax(Mat im, int x, int y){
	int i;
	int j;
	float result = -1000;
	float tmp;
...
void BinarizerAmit::structuralContrastNorm(Mat im, Mat out)
{
	int i, j;
	int up;
	float down;
...
void BinarizerAmit::structuralContrastComb(Mat scImg, Mat scNormImg, Mat result)
{
	int i, j;
	int first;
	int second;
...
void BinarizerAmit::structuralContrastMult(Mat scCombImg, Mat scNormImg, Mat result)
{
	int i, j;
	int first;
	for (i = 0; i < scNormImg.rows; i++){
...
void BinarizerAmit::binarizationProcess(Mat niBlack, Mat Mult, Mat img, Mat output)
{

}

void BinarizerAmit::postProcessing(Mat weak, Mat output)
{
...
void BinarizerAmit::reconstruction(Mat weakImg, Mat strongImg, Mat finalResult)
{
	int i, j;
	int k;
